...
jQuery.fn.calendario = function(options){
	...
	this.each(function(){
		...
		//criando div
    if (options.datavalidadecartao=='0') {
    ...
    } else if (options.datavalidadecartao=='1') {
  		jQuery('body').append('<div class="calendario" id="'+idCalendario+'"><a href="#" class="fechar" title="Fechar">X</a><a href="#" class="bt_controle_mes bt_voltar_mes">&laquo;</a><p class="nome_mes">mês ano</p><a href="#" class="bt_controle_mes bt_avancar_mes">&raquo;</a></div>');
    }
      ...
      $('#'+idCalendario+' p.nome_mes').click(function(){ 
    				var mes = (1 + parseInt($.trim($(this.parentNode).find('input[name="calendarioMes"]').val()),10)).toString();
    				if(mes.length==1)mes = '0'+mes;
    				var ano = parseInt($.trim($(this.parentNode).find('input[name="calendarioAno"]').val()),10);				
          				
          				if(options.target!='' && $(options.target).size()>0){
          					var tag = $(options.target).get(0).tagName.toLowerCase();
          					if(tag=='input'){
          						$(options.target).val(mes+'/'+ano);
                      $(options.target).change();
          					} else {
          						$(options.target).html(mes+'/'+ano);
          					}
          				}				         
