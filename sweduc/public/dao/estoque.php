...
if ($action == "cadastraGrupo") {
    ...
    if ($row['cnt'] == 0) {
        ...
        $queryNomeDoGrupo = "SELECT grupo FROM estoque_grupos WHERE id=$id";
        $resultadoQuery = mysql_query($queryNomeDoGrupo);
        while ($linhaGrupo = mysql_fetch_array($resultadoQuery)) {
            $grupo = $linhaGrupo['grupo'];
        }
        $msg = "Grupo: " . $grupo . " removido.";
        $result = mysql_query($query);
    ...
    $querykit = "SELECT kit FROM estoque_kits WHERE id=$id";
    $resultadoQueryKit = mysql_query($querykit);
    while ($linhaKit = mysql_fetch_array($resultadoQueryKit)) {
        $kitDeletado = $linhaKit['kit'];
    }
    $msg = "Kit: " . $kitDeletado . " removido.";
    if ($result = mysql_query($query)) {
    ...
    $queryPegaKit = "SELECT kit FROM estoque_kits WHERE id=$addidkit";
    $resultadoKit = mysql_query($queryPegaKit);
    while ($linhaKit = mysql_fetch_array()) {
        $kitInserido = $linhaKit['kit'];
    }
    if ($result = mysql_query($query)) {
        echo "blue|Material adicionado ao kit.|";
        $msg = "Material adicionado ao kit : " . $kitInserido . ".";
        $status = 0;
    } else {
    ...
    $queryKitdeletado = "SELECT kit FROM estoque_kits WHERE id =$id";
    $resultadoKitDeletado = mysql_query($queryKitdeletado);
    while ($linhaKitdeletado = mysql_fetch_array($resultadoKitDeletado)) {
        $kitDeletadonome = $linhaKitdeletado['kit'];
    }
    $msg = "Material removido do kit: " . $kitDeletadonome . ".";
    if ($result = mysql_query($query)) {
    ...
    $queryPegarContagem = "SELECT unidadescontagem FROM estoque_unidadescontagem WHERE id=$id";
    $resultadoQuery = mysql_query($queryPegarContagem);
    while ($linhaContagem = mysql_fetch_array($resultadoQuery)) {
        $contagem = $linhaContagem['unidadescontagem'];
    }
     $msg = "Unidade de Contagem " . $contagem . " foi removida.";
    if ($result = mysql_query($query)) {
    ...
    $queryEstoqueDeletado = "SELECT descricao,codigo FROM estoque WHERE id=$id";
    $resultadoEstoqueQuery = mysql_query($queryEstoqueDeletado);
    while ($linhaEstoque = mysql_fetch_array($resultadoEstoqueQuery)) {
        $codigoDeletado = $linhaEstoque['codigo'];
        $descricaoDeletada = $linhaEstoque['descricao'];
    }
    $msg = "Material com o código " . $codigoDeletado . " e a descrição " . $descricaoDeletada . " foi removido.";
    if ($result = mysql_query($query)) {
    ...
    if ($tipo == 0) {
        ...
        foreach ($estoque_unidade as $eu) {
            ...
            if (isset($quantidade[$k]) && $quantidade[$k] > 0) {
                ...
                $getEstoque = "SELECT * FROM estoque_movimentacao WHERE idestoque=" . $idestoque . " AND estoque_unidade=" . $eu . " AND movimentacao=1";

                $resEstoque = mysql_query($getEstoque);


                if (!mysql_num_rows($resEstoque)) {
                    $query = "INSERT INTO estoque_movimentacao(idestoque, idunidadedestino, idterceiro, quantidade, dataoperacao, idfuncionario, datamovimentacao, valormovimentacao, tempoentrega, motivo, estoque_unidade, movimentacao)
                    VALUES ('$idestoque','$idunidadedestino','$idterceiro', $quant, '$hoje', '$idfuncionario', '$datamovimentacao', '$valormovim', '$tempodeentrega', '$motivo','$eu', 1);";
                    ...
                    $novaquant = $estoqueAtual['quantidade'] + $quant;
                    $novovalormov = $estoqueAtual['valormovimentacao'] + $valormovim;

                    $setVal = "";
                    if ($novaquant != '') {
                        $setVal .= "quantidade = " . $novaquant . ", ";
                    }
                    if ($novovalormov != '') {
                        $setVal .= "valormovimentacao =" . $novovalormov . ", ";
                    }
                    if ($tempodeentrega != '') {
                        $setVal .= " tempoentrega=" . $tempodeentrega . ", ";
                    }
                    ...
                    //echo $updEstoque;
                    if ($result = mysql_query($updEstoque)) {
                        $erroexec[] = 0;
                    } else {
                        $erroexec = 1;
                    }
        ...
        // manipula estoque
        $getEstoque = "SELECT * FROM estoque_movimentacao WHERE idestoque=" . $idestoque . " AND estoque_unidade=" . $idunidadedestino . " AND movimentacao=1";
        $resEstoque = mysql_query($getEstoque);
        ...
        if (!mysql_num_rows($resEstoque)) {
            ...
            $estoqueAtual = mysql_fetch_array($resEstoque, MYSQL_ASSOC);
            $novaquant = $estoqueAtual['quantidade'] - $quantidade;
        ...
        $query = "INSERT INTO estoque_movimentacao(idestoque, idunidadedestino, idterceiro, quantidade, dataoperacao, idfuncionario, datamovimentacao, valormovimentacao, tempoentrega, motivo, movimentacao) VALUES ('$idestoque','$idunidadedestino','$idterceiro', $quantidade, '$hoje', '$idfuncionario', '$datamovimentacao', '$valormovimentacao', '$tempoentrega', '$motivo', 0);";
...
} elseif ($action == "alterarVisibilidadeGrupo") {
    if (!isset($_REQUEST['grupo_id']) || !isset($_REQUEST['visibilidade'])) {
        http_response_code(400);
        $msg = "Falha ao atualizar grupo!";
        $debug = "Não é possível alterar o grupo sem parametro de id e/ou visibilidade.";
        echo json_encode($_SESSION['login'] == 'suporte' ? compact('msg', 'debug') : compact('msg'), JSON_THROW_ON_ERROR);
        return;
    }

    $grupo_id = $_REQUEST['grupo_id'];
    $visibilidade = $_REQUEST['visibilidade'] == "true" ? 1 : 0;

    $query = "UPDATE estoque_grupos SET disponivel_para_vendas = $visibilidade WHERE id = $grupo_id;";
    $res = mysql_query($query);
    $sucesso = $res && mysql_affected_rows() == 1;

    $msg = $sucesso ? "Visibilidade para venda alterada com sucesso!" : "Falha ao atualizar grupo!";
    $debug = "Query: $query | Erro: " . mysql_error();

    http_response_code($sucesso ? 203 : 400);
    echo json_encode($_SESSION['login'] == 'suporte' ? compact('msg', 'debug') : compact('msg'), JSON_THROW_ON_ERROR);
    salvaLog($idfuncionario, basename(__FILE__), $action, $status, json_encode($_REQUEST, JSON_THROW_ON_ERROR), $msg);
}
