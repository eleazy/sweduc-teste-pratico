...
class SMTP
{
     ...
     * @return boolean True on success.
     */
    protected function sendCommand($command, $commandstring, int|array $expect)
    {
        if (!$this->connected()) {
        ...
        if (!in_array($code, (array)$expect)) {
            $this->error = ['error' => "$command command failed", 'smtp_code' => $code, 'detail' => substr($this->last_reply, 4)];
            $this->edebug(
    ...
    {
        $this->error = ['error' => 'The SMTP TURN command is not implemented'];
     ...
     */
    public function client_send($data): int|bool
    {
     ...
     */
    public function setDebugOutput(string|callable $method = 'echo')
    {
