...
function simple_curl($url, $post = [], $get = [])
{
    $url = explode('?', $url, 2);
    if (count($url) === 2) {
        $temp_get = [];
...
$html = simple_curl('http://m.correios.com.br/movel/buscaCepConfirma.do', ['cepEntrada' => $cep, 'tipoCep' => '', 'cepTemp' => '', 'metodo' => 'buscarCep']);
...
echo $dados['logradouro'] . "|" . $dados['bairro'] . "|" . $dados['cidade'] . "|" . $dados['uf'] . "|" . $dados['cep'];
