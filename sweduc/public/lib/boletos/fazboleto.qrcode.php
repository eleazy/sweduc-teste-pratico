...
if (!function_exists('fqrcode')) {
    function fqrcode($valor)
    {
        ...
        echo '<img src="data:image/png;base64, ' . base64_encode($qrCode->writeString()) . '" alt="" width="110" height="110" border=0 style="position: absolute;top: 164px; right: 308px;">';
    }
}
...
if (intval($row['webhook_boleto_criado']) > 0) {
    $queryb = "SELECT RR.tipo
    FROM contasbanco C
    INNER JOIN remessa_retorno RR ON (C.remessa_tipo = RR.id)
    WHERE C.id = " . $row['id'] ;
    ...
    //QR CODE
    $queryQ = "SELECT emvqrcode FROM pix WHERE alunos_fichafinanceira_id = $idl LIMIT 1";
    ...
    if ($rowb['tipo'] == "400@itaupix") {
        require "include/layout_itau_pix.php";
    }

    if ($rowb['tipo'] == "240@santanderpix"){
        require "include/layout_santander_pix_qrcode.php";
    }
