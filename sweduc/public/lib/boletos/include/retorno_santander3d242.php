...
if ($numerobanco!="033") { ?>
    ...
    $agencia = substr($header_arq,53,4);//."-".substr($header_arq,36,1);
    $contacorrente =substr($header_arq,58,12)+0;//."-".substr($header_arq,46,1);
    // $contacorrente_dig = substr($contacorrente,0,-1).'-'.substr($contacorrente,-1,1);//."-".substr($header_arq,46,1);
    $contacorrente_dig = substr($header_arq,58,12).'-'.substr($header_arq,70,1);//."-".substr($header_arq,46,1);
    ...
    <div id="content">
        ...
        <div id="table-content">
            ...
            if ($row1['cnt']>0) echo "<h1>Esse arquivo já foi enviado</h1>";
                ...
                if (!isset($idrelaod)){
                    $query2="INSERT INTO financeiro_retornos (agencia, conta, banco, razaosocial, cnpj, nomearquivo, dataupload, arquivoupload, bancocod) VALUES ('$agencia', '$contacorrente', 'SANTANDER', '$razaosocial', '$cnpj', '$arquivo', '$agora', '" . $arquivoUpload . "', 'santander-242-3d')";
                        ...
                        <td style="padding:30px;">
                            <table border="1" width="100%" cellpadding="0" cellspacing="0"  id="product-table">
                                ...
                                for ($i=2; $i<$numLotes+2; $i++) {
                                    ...
                                    if(substr($segmentoT,13,1)!='T') continue;

                                    $nossonumero_ = trim(substr($segmentoT,37,20));
                                    $nossonumero = substr($nossonumero_,0,-1)+0;
                                    ...
                                    $tarifas = (substr($segmentoT,198,13)+0).','.(substr($segmentoT,211,2));
                                            ...
                                            */
                                            ...
                                            if ($status == 6 || $status == 7 || $status == 8 || $status == 17) {
                                                ...
                                                if( $v_pago >= $v_titulo) {
                                                    ...
                                                    if ($debug) echo $query4." ".$result4."<br />";
