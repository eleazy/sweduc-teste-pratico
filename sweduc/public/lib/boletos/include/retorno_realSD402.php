    ...
    <div id="content">
        ...
        <div id="table-content">
            ...
            if ($row1['cnt']>0) echo "<h1>Esse arquivo já foi enviado</h1>";
                ...
                if (!isset($idrelaod)){
                    $query2="INSERT INTO financeiro_retornos (agencia, conta, banco, razaosocial, cnpj, nomearquivo, dataupload, arquivoupload, bancocod) VALUES ('$agencia', '$contacorrente', 'ITAU', '$razaosocial', '$cnpj', '$arquivo', '$agora', '" . $arquivoUpload . "', 'realSD-402')";
                ...
                <table border="1" width="100%" cellpadding="0" cellspacing="0"  class="table table-striped">
                    ...
                    for ($i=1; $i<$numLotes+1; $i++) {
                        ...
                        $nossonumero = substr($transacao,44,14)+0; // 34,15)+0 SEM O DÍGITO FINAL
                        /*
                                ...
                                */
                                ...
                                if($valor > 0) {
                                    if (($valor - ((($valor) / 100) * $baixa_min)) <= ($valorpago + $tarifavalor)) {
                                        ...
                                        $query4 = "UPDATE alunos_fichafinanceira SET idfuncionario=$idfuncionario, situacao=6, datarecebimento='$datapagamento', desconto='" . ($descontosTotal + $tarifavalor) . "', juros='$juros', valorrecebido='$valorpago' WHERE situacao=0 AND titulo='$nossonumero'"; // gambi real 400
                                        ...
                                        echo "<br /><font color='red' data='".$valor."-".$baixa_min."-".$valorpago."-".$tarifavalor."-".($valor - ((($valor) / 100) * $baixa_min))."-".($valorpago + $tarifavalor)."'><b>Valor pago abaixo do esperado.</b></font>";
                                    }
                                }else{
