...
class gd
{
  ...
  /** Returns an array. Element 0 - GD resource. Element 1 - width. Element 2 - height.
     ...
     * @return array */
    protected function build_image(mixed $image)
    {
        ...
        if ($image instanceof gd) {
            ...
            [$key, $width][1] = current($image);
            [$key, $width]['value'] = current($image);
            [$key, $width][0] = key($image);
            [$key, $width]['key'] = key($image);
            next($image);
            [$key, $height][1] = current($image);
            [$key, $height]['value'] = current($image);
            [$key, $height][0] = key($image);
            [$key, $height]['key'] = key($image);
            next($image);
  ...
  /** Parameter $image can be:
     ...
     * @return gd */
    public function __construct(mixed $image, $bigger_size = null)
    {
  ...
  /** Resize the given image source (GD, gd object or image file path) to fit in the own image.
     ...
     * @return bool */
    public function resize_crop(mixed $src)
    {
  ...
  /** Neka si predstavim vyobrazhaem pravoygylnik s razmeri $width i $height.
    ...
    * @return bool */
    ...
    public function gd_color()
    {
        ...
        if ((is_countable($args[0]) ? count($args[0]) : 0) == 3) {
            [$r, $g, $b] = $args[0];
