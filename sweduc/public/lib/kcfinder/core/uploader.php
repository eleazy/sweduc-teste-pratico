...
/** Settings which can override default settings if exists as keys in $config['types'][$type] array
  * @var array */
    protected $typeSettings = ['disabled', 'theme', 'dirPerms', 'filePerms', 'denyZipDownload', 'maxImageWidth', 'maxImageHeight', 'thumbWidth', 'thumbHeight', 'jpegQuality', 'access', 'filenameChangeChars', 'dirnameChangeChars', 'denyExtensionRename', 'deniedExts'];
...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    public function __construct()
    {
        ...
        if (
            ...
            foreach ($_CONFIG['_sessionVar'] as $key => $val) {
                if ((!str_starts_with($key, "_")) && isset($_CONFIG[$key])) {
                    $this->config[$key] = $val;
                }
            }
        ...
        // ABSOLUTE & RELATIVE
            ...
            $this->config['uploadURL'] = (str_starts_with($this->config['uploadURL'], "/"))
    ...
    protected function checkUploadedFile(array $aFile = null)
    {
        ...
        if (is_array($file['name'])) {
            foreach ($file['name'] as $i => $name) {
                $return = $this->checkUploadedFile(['name' => $name, 'tmp_name' => $file['tmp_name'][$i], 'error' => $file['error'][$i]]);
                if ($return !== true) {
                    return "$name: $return";
                }
            }
        ...
        elseif (str_starts_with($file['name'], ".")) {
    ...
    protected function checkInputDir($dir, $inclType = true, $existing = true)
    {
        ...
        if ((str_starts_with($dir, ".")) || (str_starts_with(basename($dir), "."))) {
            return false;
        }
    ...
    protected function validateExtension($ext, $type)
    {
        ...
        if (!strlen($exts) || str_starts_with($exts, "*")) {
            return true;
        }

        if (str_starts_with($exts, "!")) {
    ...
    protected function makeThumb($file, $overwrite = true)
    {
        ...
        if (
            ($gd->get_width() <= $this->config['thumbWidth']) &&
