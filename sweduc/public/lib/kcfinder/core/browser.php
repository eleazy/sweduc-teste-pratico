...
class browser extends uploader
{
    ...
    public function __construct()
    {
        ...
        $files = dir::content($this->config['uploadDir'], ['types' => "file", 'pattern' => '/^.*\.zip$/i']);
    ...
    public function action()
    {
        ...
        if ($this->config['disabled']) {
            ...
            if (
                in_array($act, ["browser", "upload"]) ||
                (str_starts_with($act, "download"))
            ) {
                $this->backMsg($message);
            } else {
                header("Content-Type: text/plain; charset={$this->charset}");
                die(json_encode(['error' => $message], JSON_THROW_ON_ERROR));
            }
        }
    ...
    protected function act_init()
    {
        ...
        $data = ['tree' => &$tree, 'files' => &$files, 'dirWritable' => $dirWritable];
        return json_encode($data, JSON_THROW_ON_ERROR);
    }

    protected function act_thumb()
    {
        $this->getDir($this->get['dir']);
        ...
        if (!is_file($file) || !is_readable($file)) {
            ...
            $browsable = [IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG];
            if (
    ...
    {
        return json_encode(['dirs' => $this->getDirs($this->postDir())], JSON_THROW_ON_ERROR);
    }
    ...
    protected function act_chDir()
    {
        ...
        $dirWritable = dir::isWritable("{$this->config['uploadDir']}/{$this->session['dir']}");
        return json_encode(['files' => $this->getFiles($this->session['dir']), 'dirWritable' => $dirWritable], JSON_THROW_ON_ERROR);
    }
    ...
    protected function act_renameDir()
    {
        ...
        return json_encode(['name' => $newName], JSON_THROW_ON_ERROR);
    }
    ...
    protected function act_upload()
    {
        ...
        if (is_array($this->file['name'])) {
            ...
            foreach ($this->file['name'] as $i => $name) {
                $return[] = $this->moveUploadFile(['name' => $name, 'tmp_name' => $this->file['tmp_name'][$i], 'error' => $this->file['error'][$i]], $dir);
            }
            return implode("\n", $return);
        } else {
    ...
    protected function act_check4Update()
    {
        ...
        if (
            ...
            return json_encode(['version' => $this->session['checkVersion']], JSON_THROW_ON_ERROR);
        }
        ...
        if (
            ...
            if (
                (false !== (  $socket = @socket_create(AF_INET, SOCK_STREAM, SOL_TCP)  )) &&
        ...
        if (isset($ver) && preg_match($pattern, $ver)) {
            ...
            return json_encode(['version' => $ver], JSON_THROW_ON_ERROR);
        } else {
            return json_encode(['version' => false]);
        }
    }
    ...
    protected function getFiles($dir)
    {
        ...
        foreach ($files as $file) {
            ...
            $return[] = ['name' => stripcslashes($name), 'size' => $stat['size'], 'mtime' => $stat['mtime'], 'date' => @strftime($this->dateTimeSmall, $stat['mtime']), 'readable' => is_readable($file), 'writable' => file::isWritable($file), 'bigIcon' => $bigIcon, 'smallIcon' => $smallIcon, 'thumb' => $thumb, 'smallThumb' => $smallThumb];
        }
        return $return;
    }
    ...
    {
        if ((str_starts_with(basename($dir), ".")) || !is_dir($dir) || !is_readable($dir)) {
            return false;
        }
        $dirs = dir::content($dir, ['types' => "dir"]);
        if (is_array($dirs)) {
            foreach ($dirs as $key => $cdir) {
                if (str_starts_with(basename($cdir), ".")) {
                    unset($dirs[$key]);
                }
            }
        ...
        $writable = dir::isWritable($dir);
        $info = ['name' => stripslashes(basename($dir)), 'readable' => is_readable($dir), 'writable' => $writable, 'removable' => $removable && $writable && dir::isWritable(dirname($dir)), 'hasDirs' => $hasDirs];
