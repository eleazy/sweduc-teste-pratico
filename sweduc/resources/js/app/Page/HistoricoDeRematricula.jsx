...
function Table({alunos, search, setDocumentFromRematricula}) {
  ...
  const content = (rematricula) => [
    tituloAluno(rematricula),
    situacao(rematricula['etapa_atual']),
    contextoAcademico(rematricula),
    ...documentos.map(doc => {
      const alunoDocumento = rematricula.documentos.find(rd => rd.documento == doc)

      let status = 'Não Entregue'

      if (alunoDocumento) {
        status = 'Em Avaliação'

        if (alunoDocumento.pivot.aprovado_em) {
          status = 'Aprovado'
        }

        if (alunoDocumento.pivot.rejeitado_em) {
          status = 'Reprovado'
        }
      }
...
export default function HistoricoDeRematricula() {
  ...
  function search() {
    setDownloadLink('/api/v1/academico/historico-de-rematricula.xls?' +
    ...
    <div className="bg-gray-200 border-t-2 border-white">
      <div className="container mx-auto p-5">
        ...
        <form onSubmit={handleFormUpdate} className="bg-white p-4 my-2 shadow rounded">
          <ContextoAcademico unidades={true} turmas={false} series={false} cursos={false}/>
          ...
          <div className="flex flex-wrap py-2 -m-2">
            ...
            <div className="p-2 w-1/2 md:w-1/4">
              ...
              <select id="etapa_atual" name="etapa_atual" className="form-select form-element">
